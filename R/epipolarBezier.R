epipolarBezier <- function(p, cal.coeff, undistort.coeff = NULL, distort.coeff = NULL, 
	img.size = NULL, view = 2, self = FALSE){
	...
	# If first view is target, flip all inputs
	if(view == 1){
		cal.coeff <- cal.coeff[, 2:1]
		undistort.coeff <- undistort.coeff[2:1, ]
		distort.coeff <- distort.coeff[2:1, ]
		img.size <- img.size[2:1, ]
	}
	...
	if(is.null(undistort.coeff)){
		...
		xpts <- seq(0-img.size[2,'w']*0.1, img.size[2,'w']*1.1, length=4)
		...
		# Undistort point
		up <- undistort(p, img.size[1, ], center=undistort.coeff[1, c('cx', 'cy')], 
			k=undistort.coeff[1, c('k1', 'k2', 'k3')], p=undistort.coeff[1, c('p1', 'p2')])

		# Get epipolar line of undistorted point
		elineRu <- dltEpipolarLine(p=up, cal.coeff1=cal.coeff)

		# Get points spanning x-range of image for sampling of points along epipolar line
		xpts <- seq(0-img.size[2,'w']*0.1, img.size[2,'w']*1.1, length=50)

		# Generate points along undistorted epipolar line
		epointsRu <- cbind(xpts, xpts*elineRu$m + elineRu$b)

		# Distort epipolar line points
		epointsRd <- undistort(epointsRu, img.size[2, ], center=distort.coeff[2, c('cx', 'cy')], 
			k=distort.coeff[2, c('k1', 'k2', 'k3')], p=distort.coeff[2, c('p1', 'p2')])
